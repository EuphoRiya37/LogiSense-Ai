...
export default function Predictions() {
  ...
  const fimpData =
    modelPerf?.eta_feature_importance?.XGBoost?.slice(0, 10).map((f: any) => ({
  ...
  return (
    <div className="space-y-6 max-w-[1300px]">
      <SectionHeader
        ...
        subtitle="ETA estimation + delay risk analysis powered by XGBoost / LightGBM / Random Forest ensemble"
      />
          ...
          <div className="glass-card p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <NumberInput
                  value={form.scheduled_shipping_days}
                  onChange={update('scheduled_shipping_days')}
                  min={1}
                  max={30}
                />
              </div>
              ...
              <div>
                ...
                <Select
                  ...
                  onChange={update('customer_segment')}
            ...
            >
              {isPending ? (
                <>
                  <Spinner size={16} color="#020510" /> Predicting…
                </>
              ) : (
                <>
          ...
          {!result && !isPending && (
            <div className="glass-card p-10 flex flex-col items-center justify-center text-center h-64">
              ...
              <div className="text-slate-500 font-mono text-sm">
                Configure shipment parameters and click
                <br />
          ...
          {result && !isPending && (
            <>
              <div className="grid grid-cols-2 gap-4">
                ...
                >
                  ...
                  <div className="mt-3 relative h-2 rounded-full bg-cyan-500/10">
                    ...
                    />
                    <div
                      className="absolute top-1/2 -translate-y-1/2 w-2 h-4 rounded bg-cyan-400"
                      style={{
                        left: `${(result.eta_days / (result.confidence_upper * 1.2)) * 100}%`,
              ...
              {result.weather_adjustment && (
                <div
                  className="glass-card p-4 fade-in-up flex items-center gap-3"
                  ...
                  {Object.entries(result.individual_predictions).map(([model, val], i) => {
                    ...
                    return (
                      <div key={model} className="flex items-center gap-3">
                        <div className="w-20 text-xs font-mono" style={{ color: colors[i] }}>
                          {model}
                        </div>
                        <div className="flex-1">
                          <ProgressBar
                            value={val}
                            max={Math.max(...Object.values(result.individual_predictions)) * 1.2}
                            color={colors[i]}
                            showLabel={false}
                          />
                        </div>
          ...
          {fimpData.length > 0 && (
            <div className="glass-card p-5">
              <div className="flex items-center justify-between mb-3">
                <div className="stat-label">Top Predictive Features (XGBoost)</div>
                <div className="text-[10px] font-mono text-slate-600 max-w-xs text-right">
                  "speed × scheduled" = shipping class speed multiplied by planned days — strongest ETA signal
                </div>
              </div>
