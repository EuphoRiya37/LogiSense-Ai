...
CARGO_PROFILES = {
    "sugarcane": {
        ...
        "hazmat": False,
        "icon": "🌾",
        "spoilage_penalty_pct_per_hour": 4.0,  # 4% value loss per hour over limit
    ...
    "frozen_food": {
        ...
        "notes": "Reefer truck required, must maintain -18°C ± 3°C chain"
    },
    "fresh_produce": {
        ...
        "notes": "Cold chain 2–8°C required, keep upright"
    },
    "eggs": {
        "label": "Eggs / Glassware",
        "category": "fragile",
...
VEHICLE_CAPABILITIES = {
    "standard_van": {
        ...
        "max_weight_kg": 1200,
        "refrigeration": None,
        "shock_rating": "medium",   # g tolerance
    ...
    "reefer_van": {
        "label": "Refrigerated Van (Reefer)",
        "max_weight_kg": 1000,
        "refrigeration": "chilled",  # 2–8°C
        "temp_range_c": (2, 8),
        "shock_rating": "medium",
        "hazmat_certified": False,
        "flatbed": False,
    ...
    "frozen_truck": {
        ...
        "max_weight_kg": 8000,
        "refrigeration": "frozen",  # -25 to -15°C
        "temp_range_c": (-25, -15),
        "shock_rating": "medium",
        "hazmat_certified": False,
        "flatbed": False,
    ...
    "low_vibration_truck": {
        ...
        "max_weight_kg": 5000,
        "refrigeration": None,
        "shock_rating": "low",  # <0.3g
        "hazmat_certified": False,
        "flatbed": False,
        "terrain": ["highway", "urban"],  # avoids rough roads
        "avg_speed_kph": 70,
        "co2_kg_per_km": 0.40,
        "cost_per_km": 1.50,
        "icon": "🚛🛡️"
...
TERRAIN_PROFILES = {
    ...
    "rural":       {"vibration_factor": 1.8, "speed_factor": 0.75, "accessible_vehicles": ["standard_van","reefer_van","frozen_truck","hazmat_truck","flatbed_truck","low_vibration_truck"]},
...
def analyze_cargo(
    ...
    - feasibility per vehicle type
    - recommended vehicle
    - latest safe dispatch time
    - spoilage/damage risk scores
    - SLA warnings
    """
    ...
    for vid in available_vehicles:
        ...
        warnings = []
        feasible = True
        score = 100  # starts at 100, deductions applied
        ...
        if profile.get("hazmat") and not veh.get("hazmat_certified"):
            issues.append("Hazmat Class " + profile.get("hazmat_class","?") + " requires ADR-certified vehicle")
            feasible = False
            score -= 45
        ...
        if cargo_shock > veh_shock:
            ...
            warnings.append(f"Vibration risk: cargo needs shock rating ≥{profile['shock_sensitivity']}, vehicle provides {veh['shock_rating']}")
            score -= gap * 15

        # ── ETA & spoilage calculation ─────────────────────────────────────────
        ...
        # ── Latest safe dispatch time ─────────────────────────────────────────
        if max_hours < 720:
            latest_dispatch = dispatch_dt + timedelta(hours=max_hours - transit_hours)
            time_buffer_h = max(0, max_hours - transit_hours)
        else:
            latest_dispatch = None
            time_buffer_h = 999

        # ── Damage probability estimate ───────────────────────────────────────
