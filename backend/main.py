...
async def lifespan(app: FastAPI):
    ...
    cached = (
        os.path.exists(os.path.join(model_path, "eta_xgboost.pkl"))
        and os.path.exists(os.path.join(model_path, "eta_gradientboost.pkl"))
        and os.path.exists(os.path.join(model_path, "eta_randomforest.pkl"))
        and os.path.exists(os.path.join(model_path, "delay_model.pkl"))
    )
    ...
    if cached:
        ...
        logger.info("Training models (first run may take some time)…")
...
def demand_forecast():
    ...
    try:
        ...
        monthly = (
            df.groupby(["order_year", "order_month"], observed=False)
            .size()
        ...
        recent_n = min(6, len(counts))
        slope = float(np.polyfit(np.arange(recent_n), np.array(counts[-recent_n:]), 1)[0])

        hist = [
            {
                "label": row["label"],
                "count": int(row["count"]),
                "smoothed": round(smoothed[i], 0),
                "type": "historical",
            }
            for i, row in monthly.iterrows()
        ]

        last_y = int(monthly.iloc[-1]["order_year"])
        last_m = int(monthly.iloc[-1]["order_month"])
        last_v = smoothed[-1]
        ...
        logger.warning(f"Demand forecast error: {e}")
        return []
...
def predict_whatif(req: WhatIfRequest):
    ...
    results = [
        {
            "label": "Baseline",
            **eta_predictor.predict(base_d),
            **delay_predictor.predict(base_d),
        }
    ]
...
def compare_modes(payload: ShipmentInput):
    ...
    for mode in modes:
        d = {
            **payload.model_dump(),
            "shipping_mode": mode,
            "scheduled_shipping_days": sched[mode],
        }
        try:
    ...
    if day_save > 0.5 and cost_diff < 50:
        rec = {
            "type": "UPGRADE",
            "message": f"Switch to {best['mode']} → save {day_save}d at +${cost_diff}",
        ...
        rec = {
            "type": "RISK_REDUCE",
            "message": f"Use {safest['mode']} → reduce delay risk by {rs:.0f}%",
            "suggested_mode": safest["mode"],
        }
    else:
        rec = {
            "type": "OPTIMAL",
            "message": f"{payload.shipping_mode} is optimal",
            "suggested_mode": payload.shipping_mode,
        }

    return {
        "modes": results,
        "current_mode": payload.shipping_mode,
        "recommendation": rec,
    }
...
async def optimize_routes_road(req: RouteRequest):
    ...
    if api_key:
        ...
        result["road_routing_note"] = (
            "Set OPENROUTESERVICE_API_KEY in .env for road-following routes"
        )
...
WEATHER_CODES = {
    ...
    3: ("Overcast", "☁️", 0.05),
    45: ("Fog", "🌫️", 0.15),
    51: ("Light drizzle", "🌦️", 0.1),
    61: ("Slight rain", "🌧️", 0.1),
    63: ("Moderate rain", "🌧️", 0.2),
    65: ("Heavy rain", "🌧️", 0.35),
    71: ("Slight snow", "🌨️", 0.25),
    73: ("Moderate snow", "🌨️", 0.4),
    75: ("Heavy snow", "❄️", 0.55),
    80: ("Showers", "🌦️", 0.15),
    82: ("Violent showers", "⛈️", 0.5),
    95: ("Thunderstorm", "⛈️", 0.6),
}
...
>>>>>>> e960e02c3296a83b67acc4c49fc3e42153cb78be
...
async def geocode(q: str):
    try:
        async with httpx.AsyncClient(
            timeout=8.0,
            headers={"User-Agent": "LogiSenseAI/1.0"},
        ) as client:
            resp = await client.get(
                "https://nominatim.openstreetmap.org/search",
                params={"q": q, "format": "json", "limit": 5},
