...
async def lifespan(app: FastAPI):
    ...
    if cached:
        ...
        logger.info("Training models (first run ~60-90s)...")
...
def demand_forecast():
    ...
    try:
        ...
        hist = [
            {"label": row["label"], "count": int(row["count"]), "smoothed": round(smoothed[i], 0), "type": "historical"}
        ...
        for o in range(1, 7):
            ...
            if m in (11, 12):
                ...
                fv *= 0.92
            forecasts.append({"label": f"{y}-{str(m).zfill(2)}", "count": None, "smoothed": None, "forecast": round(fv, 0), "type": "forecast"})
...
def revenue_at_risk():
    ...
    return {"at_risk_shipments": at_risk, "total_revenue_at_risk": round(total_risk, 2), "count": len(at_risk)}
